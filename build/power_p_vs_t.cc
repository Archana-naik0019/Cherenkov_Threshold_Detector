...
void plot_efficiency_vs_threshold_powerlaw() {
    const int N = 15;
    ...
    // --- Separate plots ---
    TCanvas *c_sep = new TCanvas("c_sep","Separate Power-Law Fits",1200,800);
    c_sep->Divide(3,2);

    TF1* fits[6]; 
    TString legendEntries[6];
    ...
    for (int i=0;i<6;i++) {
        ...
        // Power-law fit: A*x^B
        fits[i] = new TF1(Form("fit%d",i),"[0]*pow(x,[1])",1.0,3.2);
        fits[i]->SetParameters(0.1, 1.0); 
        ...
        // Get parameters
        double A = fits[i]->GetParameter(0);
        double B = fits[i]->GetParameter(1);

        // Print to terminal
        std::cout << labels[i] << " fit: Efficiency = " << A << " * Threshold^(" << B << ")" << std::endl;

        // Store legend entry with rounded params
        legendEntries[i] = Form("%s: y = %.3f * x^{%.3f}", labels[i], A, B);
    }

    // --- Overlay only trendlines ---
    TCanvas *c_overlay = new TCanvas("c_overlay","Overlay Power-Law Fits Only",800,600);

    bool first = true;
    for (int i=0;i<6;i++) {
        if (first) {
            fits[i]->SetTitle("Efficiency vs Threshold (Power-Law Fits);Threshold (GeV);Efficiency");
            fits[i]->Draw(); 
            first = false;
        } else {
            fits[i]->Draw("SAME");
    ...
    auto legend = new TLegend(0.45,0.2,0.88,0.48);
    for (int i=0;i<6;i++) legend->AddEntry(fits[i], legendEntries[i], "l");
    legend->Draw();
}
